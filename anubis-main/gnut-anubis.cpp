...
#include <thread>
#include "src/gset/gcfg_anubis.h"
...
{   
  ...
  for( int i=1; i<argc; i++ ){
    ...
    else if( !strcmp(argv[i],"-m") && i  <argc ){ thrd = true; }//多线程
    else if( !strcmp(argv[i],"-r") && i  <argc ){ redu = true; }
    else if( !strcmp(argv[i],"-e") && i  <argc ){ exml = true; }//exml格式文件的输出
  ...
  gset.app("G-Nut/Anubis", "2.3.0", "$Rev: 2843 $","(gnss@pecny.cz)", __DATE__, __TIME__);
  gset.arg( argc, argv, true, thin);//命令行参数
